...
// Normalize cell strings: convert null/undefined to '', replace NBSP-like characters with normal spaces
// and collapse multi whitespace so header comparisons are consistent.
const normalizeCellString = (val: string | number | null | undefined): string => {
    const s = String(val ?? '');
...
const isMultiScholarFormat = (rows: (string | number)[][]): boolean => {
    ...
    for (let i = 0; i < Math.min(rows.length, 10); i++) {
        const row = rows[i] || [];
        if (row.some(cell => normalizeCellString(cell) === 'שם' || normalizeCellString(cell) === 'שם האברך')) {
            return true;
        }
    }
    return false;
...
const processMultiScholarSheet = (
        ...
        for (let i = 0; i < Math.min(rows.length, 10); i++) {
            if (rows[i]) {
                for (const cell of rows[i]) {
                    ...
                    const sCell = normalizeCellString(cell);
    ...
    for (let j = 0; j < Math.max(dateRow.length, sederRow.length, typeRow.length); j++) {
        ...
        if (dateCell !== null && dateCell !== undefined && dateCell !== '') {
            ...
            if (typeof dateCell === 'number' && dateCell > 25569) { // Excel serial date
                ...
                const dateCellStr = normalizeCellString(dateCell);
        ...
        const sederCell = normalizeCellString(sederRow[j] || '');
        if (sederCell && isNaN(Number(sederCell))) {
            currentSeder = sederCell;
        }

        const type = normalizeCellString(typeRow[j] || '');
    ...
    (settings.generalBonuses || []).forEach(b => {
        ...
        for (let j = 0; j < Math.max(dateRow.length, sederRow.length, typeRow.length); j++) {
            ...
            const header1 = normalizeCellString(rows[dateRowIndex]?.[j]);
            const header2 = normalizeCellString(rows[sederRowIndex]?.[j]);
            const header3 = normalizeCellString(rows[typeRowIndex]?.[j]);
    ...
    for (let i = dataStartRow; i < rows.length; i++) {
        ...
        Object.entries(columnMap).forEach(([colIndex, mapInfo]) => {
            ...
            const normalizedSeder = normalizeCellString(mapInfo.sederName);
            // If multiple columns map to same seder on the same day, prefer the first non-empty value
            const existingEntry = day.entries[normalizedSeder];
            const existingExit = day.exits[normalizedSeder];
            const existingBonus = day.bonuses[normalizedSeder];
            const cellIsNonEmpty = cellValue != null && normalizeCellString(cellValue) !== '';

            if (mapInfo.type === 'כניסה') {
                if (!existingEntry || normalizeCellString(existingEntry) === '') {
                    if (cellIsNonEmpty) day.entries[normalizedSeder] = cellValue;
                }
            }
            if (mapInfo.type === 'יציאה') {
                if (!existingExit || normalizeCellString(existingExit) === '') {
                    if (cellIsNonEmpty) day.exits[normalizedSeder] = cellValue;
                }
            }
            if (mapInfo.type === 'בונוס') {
                if (!existingBonus || normalizeCellString(existingBonus) === '') {
                    if (cellIsNonEmpty) day.bonuses[normalizedSeder] = cellValue;
                }
            }
        });
        ...
        scholarInfo.details = Array.from(dailyData.entries()).map(([date, data]) => {
            ...
            settings.sedarim.forEach(sederConfig => {
                ...
                const normalizedKey = normalizeCellString(sederKey);
                if (!normalizedKey || !data.entries.hasOwnProperty(normalizedKey)) return;

                const entryTimeRaw = data.entries[normalizedKey];
                const exitTimeRaw = data.exits[normalizedKey];

                const entryStr = normalizeCellString(entryTimeRaw || '');
                const exitStr = normalizeCellString(exitTimeRaw || '');
...
const processSingleScholarSheet = (
    ...
    const monthYearRow = rows.find(row => row && row.some(cell => typeof cell === 'string' && cell.includes('חודש')));
    if (monthYearRow) {
        const monthCell = monthYearRow.find(cell => typeof cell === 'string' && cell.includes('חודש'));
        if (monthCell) {
            const match = String(monthCell).match(/(\d{1,2})\/(\d{2,4})/);
            if (match) {
                const month = match[1].padStart(2, '0');
                const year = match[2].length === 2 ? `20${match[2]}` : match[2];
                monthYear = `${month}/${year}`;
    ...
    let headerRowIndex = rows.findIndex(row => row && row.some(c => normalizeCellString(c) === 'יום') && row.some(c => normalizeCellString(c) === 'כניסה'));
    if (headerRowIndex === -1) throw new Error(`לא ניתן היה למצוא את שורת הכותרות עבור ${name}.`);

    const headerRow = rows[headerRowIndex];
    const dayColIndex = headerRow.findIndex(cell => normalizeCellString(cell) === 'יום');

    const bonusColIndices: { [key: string]: number } = {};
    const bonusData: { [key: string]: number } = {};
    (settings.generalBonuses || []).forEach(b => {
        const colIndex = headerRow.findIndex(cell => normalizeCellString(cell).includes(b.name.split(' ')[1] || b.name)); // Simple matching
        if (colIndex > -1) bonusColIndices[b.name] = colIndex;
        bonusData[b.name] = 0;
    });

    const entryIndices = headerRow.map((c, i) => normalizeCellString(c) === 'כניסה' ? i : -1).filter(i => i !== -1);
    const exitIndices = headerRow.map((c, i) => normalizeCellString(c) === 'יציאה' ? i : -1).filter(i => i !== -1);

    let dataStartRow = rows.findIndex((row, i) => i > headerRowIndex && row && normalizeCellString(row[dayColIndex]).match(/\d{1,2}\/\d{1,2}/));
    ...
    for (let i = dataStartRow; i < rows.length; i++) {
        ...
        const dateMatch = String(row[dayColIndex]).match(/(\d{1,2})\/(\d{1,2})/);
        if (!dateMatch) continue;
        const day = `${dateMatch[1].padStart(2, '0')}/${dateMatch[2].padStart(2, '0')}/${monthYear.split('/')[1]}`;

        Object.keys(bonusColIndices).forEach(bonusName => {
            const bonusValue = Number(row[bonusColIndices[bonusName]]) || 0;
            bonusData[bonusName] = (bonusData[bonusName] || 0) + bonusValue;
        });
        ...
        if (Object.values(row).some(cell => normalizeCellString(cell) === 'חופש')) {
